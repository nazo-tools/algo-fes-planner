...
function phase1(host) {
  ...
  host.appendChild(
    el("p", "hint", "公演をひらいて、参加する回を選んでください。★を付けておくと、「埋める」で先に入ります。"),
  );
  ...
  for (const r of rescueSuggestions(SHOWS, S.fixed, [...S.favorites], {
    ...
    if (!r.swaps.length) {
      box.append("どの予定を動かしても入りません。どれかの予定をはずしてください。");
    } else {
...
function breakPanel() {
  ...
  const put = el(
    ...
    hit
      ...
      : DAY_LABEL[S.brk.day] + " " + S.brk.start + "–" + S.brk.end + " を" + S.brk.label + "として空ける",
  );
...
function showRow(show, opts, placed) {
  ...
  if (done) {
    ...
    box.appendChild(el("div", "dl", DAY_LABEL[cur.day] + " " + cur.start + "–" + cur.end + " に参加"));
...
function phase2(host) {
  ...
  host.appendChild(
    el("p", "wnote", "午後から行く、夕方までに帰る、といったときに使います。指定しなければ、空いている時間に入れます。"),
  );
  ...
  for (const day of DAYS()) {
    for (const g of gapsOf(day)) {
      ...
      c.appendChild(el("div", "gs", g.b - g.a + "分空いています"));
...
function sharedBar(host) {
  ...
  const back = el("button", "clr", "見るだけ（自分の予定に戻す）");
  back.onclick = () => {
    S.fixed = S.mine ?? [];
    S.shared = false;
    S.mine = null;
...
function sharePanel(host) {
  ...
  box.appendChild(
    el("p", "fine", "開いた人には「送られてきた予定」として出ます。相手が取り込むまで、相手自身の予定はそのまま残ります。"),
  );
  host.appendChild(box);
}
...
function board(host) {
  ...
  for (const d of days) {
    ...
    tot.appendChild(
      el("span", null, DAY_LABEL[d] + " " + evs.filter((f) => f.showId).length + "（空き" + idle + "分）"),
    );
  }
  ...
  for (const day of days) {
    ...
    const evs = onDay(day);
    if (!evs.length) col.appendChild(el("div", "emptyday", "まだ予定がありません"));
    for (const f of evs) {
      const show = f.showId ? byId.get(f.showId) : null;
...
function intro() {
  ...
  for (const [n, ttl, body] of [
    [
      "1",
      "押さえる",
      "これだけは外せない公演は、何時の回に参加するかまで自分で決めます。ごはんや移動、用事にあてる時間も、ここで先に空けておきます。",
    ],
    [
      "2",
      "埋める",
      "残った空き時間に入れられる公演を提案します。押さえた予定は動かしません。土日をまとめて見て、待ち時間が短くなる入れ方を選びます。",
    ],
  ]) {
    const li = el("li");
